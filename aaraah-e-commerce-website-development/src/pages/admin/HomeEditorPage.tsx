...
export default function AdminHomeEditorPage() {
  ...
  return (
    <div>
      ...
      {/* Font style */}
      <section className="mb-6 rounded-2xl border border-stone-200 bg-white p-5">
        <h2 className="mb-4 text-sm font-semibold text-stone-900">Font Style</h2>
        <p className="mb-3 text-xs text-stone-500">Changes the font used across the whole website.</p>
        <select
          value={settings.font_family}
          onChange={(e) => {
            const value = e.target.value;
            setSettings((s) => (s ? { ...s, font_family: value } : s));
            applySiteFont(value); // live preview immediately, saved on "Save Changes"
          }}
          className="w-full max-w-sm rounded-lg border border-stone-300 px-3 py-2 text-sm sm:w-auto"
        >
          {FONT_OPTIONS.map((f) => (
